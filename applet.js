...
applet = {
    ...
    page: {
        set: function(page) {
            if (applet.conf.app_data.cache === undefined) {
                applet.conf.app_data.cache = new Array();
            }
            if (applet.conf.app_data.halted === undefined) {
                applet.conf.app_data.halted = false;
            }
            if (applet.conf.app_data.halted === true) {
                console.error("A fatal error has occurred. The application is no longer responding. Please reload the page.");
                return;
            }
            var data = null;
            let cached = false;
            if (applet.conf.app_data.cache[page] !== undefined) {
                ...
                if (applet.conf.applet_loader) {
                    document.getElementById(applet.conf.applet_loader).style.width = "0%";
                }
                finish();
            } else {
                if (applet.conf.applet_loader) {
                    document.getElementById(applet.conf.applet_loader).style.width = "25%";
                }
                var xhttp = new XMLHttpRequest();
                xhttp.onreadystatechange = function() {
                    if (applet.conf.applet_loader) {
                        document.getElementById(applet.conf.applet_loader).style.width = "55%";
                    }
                    if (this.readyState === 4 && this.status === 200) {
                        if (applet.conf.applet_loader) {
                            document.getElementById(applet.conf.applet_loader).style.width = "75%";
                        }
                        data = this.responseText;
                        applet.conf.app_data.cache[page] = {
                            "pageData": data
                        };
                        applet.conf.app_data.failed_loads = 0;
                        if (applet.conf.applet_loader) {
                            document.getElementById(applet.conf.applet_loader).style.width = "90%";
                        }
                        finish();
                        ...
                        if (applet.conf.app_data.failed_loads === 0) {
                            applet.page.reload();
                        } else {
                            applet.alert.new("Fatal Error", "Attempted to load default page and failed. The application has exited.");
                            applet.conf.app_data.halted = true;
                        }
                        applet.alert.new("Error", "We've encountered a 404. This has been reported to the site Admin.");
                    }
                };
            ...
            function finish() {
                ...
                for (var i = 0; i < applet.listeners.page_change_event.length; i++) {
                    applet.listeners.page_change_event[i]({
                        page: page,
                        cached: cached
                    });
                }
                if (applet.conf.applet_loader) {
                    document.getElementById(applet.conf.applet_loader).style.width = "100%";
                    document.getElementById(applet.conf.applet_loader).style.transition = "";
                    setTimeout(() => {
                        if (applet.conf.applet_loader) {
                            document.getElementById(applet.conf.applet_loader).style.width = "0%";
                            document.getElementById(applet.conf.applet_loader).style.transition = "0.5s ease-in-out";
                        }
                    }, 1200);
                }
        ...
        reload: function(e) {
            if (e !== undefined && e === true) {
                ...
                applet.page.set(applet.conf.default_page);
                return;
            }
            if (applet.conf.app_data.halted === true) {
                return;
            ...
            applet.page.set(applet.conf.default_page);
            applet.conf.app_data.failed_loads = 1;
        }
    },
    events: {
        "hashchange": function() {
...
getElementsByAttribute = function(attr) {
    ...
    function checkChildren(elem) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (!!children[i].getAttribute(attr) === true) {
